(function initGtvMainRuntimePrimaryStateAccessorProxies(globalObj) {
    "use strict";

    function createService(deps = {}) {
        const safeDeps = (deps && typeof deps === "object") ? deps : {};
        const getMainRuntimePrimaryStateService = (
            typeof safeDeps.getMainRuntimePrimaryStateService === "function"
        )
            ? safeDeps.getMainRuntimePrimaryStateService
            : (() => null);
        ...
        function callRuntimePrimaryState(methodName, args = [], fallbackFn = null) {
            const service = getMainRuntimePrimaryStateService();
        ...
        function setIsRealtimeState(next) {
            return callRuntimePrimaryState("setIsRealtimeState", [next], () => {
        ...
        function getIsRealtimeState() {
            return callRuntimePrimaryState("getIsRealtimeState", [], () => !!getIsRealtime());
        }

        function getGlobalTimesState() {
            return callRuntimePrimaryState("getGlobalTimesState", [], () => {
        ...
        function getGlobalTimeState(slotIdx = 0) {
            return callRuntimePrimaryState("getGlobalTimeState", [slotIdx], () => {
        ...
        function setGlobalTimeState(slotIdx, value) {
            return callRuntimePrimaryState("setGlobalTimeState", [slotIdx, value], () => {
                const parsedIndex = Number(slotIdx);
                const safeSlotIdx = Number.isFinite(parsedIndex) ? Math.max(0, Math.trunc(parsedIndex)) : 0;
                const safeValue = (value instanceof Date && Number.isFinite(value.getTime()))
                    ? value
                    : new Date();
        ...
        function getUiScaleState() {
            return callRuntimePrimaryState("getUiScaleState", [], () => {
    ...
    globalObj.GTVMainRuntimePrimaryStateAccessorProxies = Object.freeze({
        createService
    });
