...
function createCalculatorContext() {
    ...
    const unixTsInput = register(createElement({ id: "unix-ts-input", type: "number", value: "" }));
    const unixTsDateOut = register(createElement({ id: "unix-ts-date-out", textContent: "-" }));
    const unixDateInput = register(createElement({ id: "unix-date-input", type: "datetime-local", value: "" }));
    const unixDateTsOut = register(createElement({ id: "unix-date-ts-out", textContent: "-" }));
...
test("unix converter performs bidirectional conversion", () => {
    ...
    expect(elements.unixTsDateOut.textContent.startsWith("1970-01-01")).toBe(true);

    elements.unixDateInput.value = "1970-01-02T00:00:00";
    elements.unixDateInput.dispatch("input");
    const expectedTs = String(Math.floor(new Date("1970-01-02T00:00:00").getTime() / 1000));
    expect(elements.unixDateTsOut.textContent).toBe(expectedTs);
});
